...
NO SHELL COMMANDS: Gather all context through your file-reading and file-search tools — read any file by path, including sources referenced by the diff. Do NOT use run_command/RunCommand for any purpose (no git, gh, grep, cat, python, etc.). Headless mode cannot approve a shell command it has not pre-allowed, and agy reaches for novel commands it cannot predict, so any shell call risks aborting the whole review with no output. If context is genuinely unavailable without a shell command, note the limitation in the finding and proceed. You may NOT write or modify any file.

NO SPECULATION: Do not report a finding that depends on a fact you cannot verify from the files in this workspace — e.g. whether an external action/package version or tag exists, or how a remote API behaves. Pinned dependency versions are resolved deterministically by CI and fail loudly, so never flag them as possibly-missing. If a claim would need a shell command or network access to confirm, omit it or mark it clearly as UNVERIFIED; never present it as a definite issue.
...
**No shell commands:** Even with redirects handled, `agy` (an agentic model) invents novel inspection commands each run — including ones that *write* scratch files or shell out to `python3` — and headless mode auto-denies any command not pre-allowed in `~/.gemini/antigravity-cli/settings.json`, aborting the review with empty stdout. No static allowlist can cover an open-ended command space, so the prompt forbids shell commands entirely and has `agy` gather context through its file-reading tools instead (verified to complete without a soft-denial). The `permissions.allow` allowlist stays as a safety net for any read command `agy` still reaches for. The trade-off: without shell verification `agy` speculates more, which is why the **NO SPECULATION** rule is paired with it — it suppresses unverifiable findings (e.g. "this pinned action version may not exist") that a shell-less reviewer would otherwise guess at. Do NOT remove either block.
...
- Permission soft-denial (`jetski: no output produced — a tool required the "command" permission...`) — the NO SHELL COMMANDS prompt above makes this rare; if it recurs, `agy` reached for a shell command anyway. Read `${CLAUDE_PLUGIN_ROOT}/docs/agy-troubleshooting.md`; the `permissions.allow` allowlist there is the safety net.
